	...
	self.setActionDefinitions({
        ...
        */
        ...
        layer_level_fade: {
			...
			callback: async (event) => {
                await self.sendGetRequest(`/pin/fadevalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Mixer_Level/${event.options.value}/${event.options.fade_time}`)
			},
        },
		layer_load_preset_on: {
			...
			options: [
                ...
                },
                {
                    ...
                    default:  "1",
                    isVisible: (options) => { return !options.preset_fade },
                },
				{
					id: 'fade_time',
					type: 'number',
			...
			callback: async (event) => {
                ...
                {
                    await self.sendGetRequest(`/pin/setvalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Presets_UsePresetSettings/true`)
                }
                else
                {
                    await self.sendGetRequest(`/pin/setvalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Presets_UsePresetSettings/false`)
                    await self.sendGetRequest(`/pin/setvalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Presets_FadeTime/${event.options.fade_time}`)
                    await self.sendGetRequest(`/pin/setvalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Presets_FadeType/${event.options.fade_type}`)
                }
		...
		layer_play_modes: {
			...
			callback: async (event) => {
                await self.sendGetRequest(`/pin/setvalue/Engine_Mix${event.options.mix}_Layer${event.options.layer}_Source_MediaPlayer_PlayMode/${event.options.playmode}`)
			},
		},
        /*
        *  MIX CONTROLS
        */
        mix_level_fade: {
			...
			callback: async (event) => {
                await self.sendGetRequest(`/pin/fadevalue/Engine_Mix${event.options.mix}_MixControls_Level/${event.options.value}/${event.options.fade_time}`)
			},
        },
		mix_load_preset_on: {
        ...
        */
		...
		pin_fade_to_value: {
			...
			callback: async (event) => {
                await self.sendGetRequest(`/pin/fadevalue/${event.options.path}/${event.options.value}/${event.options.fade_time}`)
			},
		},
		pin_reset_value: {
		...
		timeline_action: {
			...
			options: [
                ...
                },
				{
					...
					type: 'textinput',
					label: 'Timeline ID',
                    isVisible: (options) => { return !options.timeline_all },
                    default: '0' 
				},
				{
					id: 'timeline_action',
				...
				},
                {
                    ...
                    default: 'play',
                    isVisible: (options) => { return ['gocue', 'gonextcue', 'gopreviouscue'].includes(options.timeline_action) }
                },
                {
                    id: 'cue_number',
                    type: 'textinput',
                    label: 'Cue Number',
                    isVisible: (options) => { return options.timeline_action == 'gocue' },
                    default: 0 
                },
            ],
			callback: async (event) => {
